// reccurse, the filemaker of ncurses, version 0.271
...
void Show_Field_ID(Annotated_Field *tfield);
   ...
   Intro_Screen();
    ...
    if (!tryfile(dbfile)) {
     ...
     if (i==-1) {
      ...
      outrcfile << "#date&time 0 0 58 35 3 10 2 0 6 1 1 1 2 . 0 0 1 1 . " << endl << "#";
      outrcfile.close();
...
int Read_Write_db_File(int mode) // 0 read, 1 write, 2 create from file, 3 recreate from istringstream
{
  ...
  dbfileaccess.seekp(0, ios::beg);
   ...
   switch (mode) {
    case 1:
//      dbfileaccess.seekp(startofrecords+4, ios::beg);
...
int Read_Write_Current_Record(int mode) // 0 read, 1 write
{
  ...
  if (!mode) {
//    dbfileaccess.seekg(fieldposition(recordsnumber, 0), ios::beg);
...
void Show_Menu_Bar(int mode) // 0 show, 1 remove
{
  ...
  if (!mode) {
   if (recordsdemo || menubar==2) {
    ...
    printw("find results->"); }
    Change_Color(2);
  printw("record %d of %d.field %d of %d.record size:%d.field size:%d", currentrecord+1, recordsnumber, currentfield+1, fieldsperrecord, recordsize, fieldlength(records[(currentrecord*fieldsperrecord)+currentfield].text));  } }
